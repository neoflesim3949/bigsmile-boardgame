...
- **`/captain/scan` 關主掃碼**：頁面頂部常駐「進行中列表」（玩家姓名 + 模組名）。掃碼後玩家卡片**不顯示持股資訊**。主操作按鈕改為「加入進行列表（執行）」；進行列表每筆右側有「完成結算」按鈕。**完成結算必須走確認彈窗**（modal 內預覽四項變動 + 道具，再按一次「確認結算」才呼叫 `applyQuickAction`）；防止關主誤觸把獎勵發給錯的人或誤扣分。
